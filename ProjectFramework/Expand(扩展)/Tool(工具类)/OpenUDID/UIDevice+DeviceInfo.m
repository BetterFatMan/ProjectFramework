...
+ (NSString *)deviceVersion
{
    ...
    NSString *machine = @"1";
    double screenWidth = [UIScreen mainScreen].currentMode.size.width;
    double screenHeight = [UIScreen mainScreen].currentMode.size.height;
    if (screenWidth == 750.0 && screenHeight == 1334.0)
    {
            ///iPhone 6
        machine = @"3";
    }
    else if (screenWidth == 640.0 && screenHeight == 1136.0)
    {
            ///iphone5,5s
        machine = @"2";
    }
    else if (screenWidth == 640.0 && screenHeight == 960.0)
    {
            ///iPhone4，4s
        machine = @"1";
    }
    else if (screenWidth == 1242.0 && screenHeight == 2208.0)
    {
            ///iPhone 6s
        machine = @"4";
    }
...
+ (BOOL)isUpIphone5
{
    ...
    if ([machine isEqualToString:@"iPhone3,1"] || [machine isEqualToString:@"iPhone4,1"] || [machine isEqualToString:@"iPod5,1"])
    {
        return NO;
    }
    return YES;
...
+ (NSString *)userPreferLanguages
{
    ...
    NSString *preferLanguage = [languagesArray objectAtIndexSafe:0];
