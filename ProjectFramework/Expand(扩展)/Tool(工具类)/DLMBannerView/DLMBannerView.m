...
#import "DLMBannerView.h"
#import <SDWebImage/UIImageView+WebCache.h>


#define DEFAULT_TIMEINTERVAL  5.0f  // 默认滚动时间间隔
#define MIN_TIMEINTERVAL  0.3f  // 最小的滚动时间间隔
#define REUSE_IDENTIFIER  @"DLMBannerCollectionCell"


@interface DLMWeaker : NSObject
...
@interface DLMBannerCollectionCell : UICollectionViewCell

@property (nonatomic, strong, readonly) UIImageView *imageView;

@end

@implementation DLMBannerCollectionCell
...
@interface DLMBannerView()<UICollectionViewDelegate,
...
- (void)viewInit {
    ...
    // 设置默认属性
    _imageViewMode = UIViewContentModeScaleToFill;
    _timeInterval = DEFAULT_TIMEINTERVAL;
    _autoScroll = YES;
    _scrollDirection = DLMBannerViewScrollDirectionLeft;
    _pageCount = 0;
    ...
    // 注册Cell
    [self.collectionView registerClass:[DLMBannerCollectionCell class]
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView
    ...
    DLMBannerCollectionCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:REUSE_IDENTIFIER forIndexPath:indexPath];
    ...
    NSString *imagePath = [self.dataSource bannerView:self imagePathAtPage:[self pageWithIndexPath:indexPath]];
    [cell.imageView sd_setImageWithURL:[NSURL URLWithString:imagePath] placeholderImage:self.placeholder];
...
- (void)reloadData {
    ...
    CGSize pageControlSize = [self.pageControl sizeForNumberOfPages:_pageCount];
    CGFloat pageControlOffsetX = CGRectGetWidth(self.frame) - pageControlSize.width - 10;
    self.pageControl.frame = CGRectMake(pageControlOffsetX, CGRectGetHeight(self.bounds) - 26,
                                        pageControlSize.width+5, 26);
    
    if (_pageCount > 1) {
        // 根据滚动方向，移动到第一张或者最后一张图片的位置
        NSInteger item = (self.scrollDirection == DLMBannerViewScrollDirectionLeft) ?
        1 : _pageCount;
...
- (void)addTimer {
    ...
    DLMWeaker *target = [[DLMWeaker alloc] initWithObject:self];
...
- (void)setDataSource:(id<DLMBannerViewDataSource>)dataSource {
