...
- (void)setPhoto:(MLPhotoBrowserPhoto *)photo{
    ...
    if (photo.photoURL.absoluteString.length) {
        ...
        if (photoRange.location != NSNotFound){
            ...
            __weak typeof(_photoImageView) _wPhotoImage = _photoImageView;
            ...
            [_photoImageView sd_setImageWithURL:photo.photoURL placeholderImage:thumbImage options:SDWebImageRetryFailed progress:^(NSInteger receivedSize, NSInteger expectedSize) {
                ...
                if (image.size.width < boundsSize.width*0.9 || image.size.height < boundsSize.height*0.9) {
                    
                    CGSize imageSize = image.size;
                    
                    if (imageSize.width==0 || imageSize.height==0) {
                        _wPhotoImage.image = image;
                    } else {
                        ...
                        UIImage *tempImage = [_wself imageWithImage:image scaledToSize:CGSizeMake(imageSize.width*minScale, imageSize.height*minScale)];
                        _wPhotoImage.image = tempImage;
                    }
