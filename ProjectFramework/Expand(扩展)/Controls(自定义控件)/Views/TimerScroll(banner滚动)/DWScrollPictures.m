...
/** 是否是启动延迟加载 */
@property (assign, nonatomic) BOOL              afterSlide;
...
- (void)dw_SetNewFeaturesView:(UIView *)view delegate:(id)delegate imageName:(NSArray *)imageNameArray pageImageView:(void (^)(UIView *PageImageView, int imageCount, int imageAllCount))pageImageView {
    ...
    for (int i = 0; i < [imageNameArray count]; i ++) {
        ...
        imageView.left = i * scrollView.width;
    ...
    pageControl.top = view.height - 100;
...
//             [self.tapGesture dw_SwipeGestureType:DWTapGesture Target:self Action:@selector(dw_tapGesture) AddView:imageView BackSwipeGestureTypeString:^(NSString * _Nonnull backSwipeGestureTypeString) {}];
    ...
    //设置位置
    pageControl.centerX = view.centerX;
    pageControl.top = view.height - pageY;
...
- (void)dw_SetNetworkingShufflingFigureView:(UIView *)view sizeY:(CGFloat)sizeY  height:(CGFloat)height pageY:(CGFloat)pageY imageLinkArray:(NSArray *)imageLinkArray timeInterval:(NSTimeInterval)timeInterval animateTimer:(NSTimeInterval)animateTimer pageImageView:(void (^)(UIView *PageImageView, int imageCount, int imageAllCount))pageImageView {
    ...
    [self.shufflingFigureImageLinkArray removeAllObjects];
    
    BOOL isHasData = imageLinkArray.count;
    ...
    if (imageLinkArray.count > 1) {
        ...
        [self.shufflingFigureImageLinkArray addObjectsFromArray:(imageLinkArray.count?imageLinkArray:@[@""])];
    ...
    for (int i = 0; i < [self.shufflingFigureImageLinkArray count]; i ++) {
        ...
        imageView.backgroundColor = [UIColor colorWithRed:(0xed)/255.0f green:(0xed)/255.0f blue:(0xf3)/255.0f alpha:1];
        
        if (isHasData) {
            
            NSDictionary *dic = self.shufflingFigureImageLinkArray[i];
            if ([dic isKindOfClass:[NSDictionary class]] && dic.allKeys.count) {
                
                NSString *urlString = [dic safeBindStringValue:@"imageUrl"];
                
                imageView.imageUrl = [dic safeBindStringValue:@"url"];
                
                if (urlString.length) {
                    
                    NSURL *url = [NSURL URLWithString:urlString];
                    
//                    [imageView sd_setImageWithURL:url placeholderImage:[UIImage imageNamed:@"image_homebanner"]];
        ...
        imageView.left = i * scrollView.width;
        imageView.image = [UIImage imageNamed:@"image_homebanner"];
        
        if (pageImageView && isHasData) {
    ...
    //设置位置
    CGFloat widths = pageControl.numberOfPages*12+30;
    pageControl.frame = CGRectMake(view.width-widths, scrollView.bottom - 15, widths, 15);
    ...
    if (self.isbool && self.slide) {
        ...
        if (!self.afterSlide) {
            ...
            dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(2.0 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
                [_wself.shufflingTimer setFireDate:[NSDate distantPast]];
                
                _wself.afterSlide = NO;
            });
        }
...
- (void)dw_dismissShuffling {
    ...
    if ([_shufflingTimer isValid]) {
        [_shufflingTimer invalidate];
